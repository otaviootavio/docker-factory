...
class GCPClient:
    def __init__(self):
        ...
        try:
            ...
            missing_fields = [
                field for field in required_fields if field not in credentials_json]
            if missing_fields:
                raise ValueError(f"Missing required fields in credentials: {
                                 missing_fields}")
            ...
            raise ValueError(
                f"Invalid JSON in GCP_SERVICE_ACCOUNT_KEY: {str(e)}")
